...
def run(T,initial_pos,initial_bearing,geno,plot_flag):
    ...
    #cut1= geno[1]; #left motor to right sensor
    w_rl = geno[1]; #right motor to left sensor
    #cut2 = geno[3]; #right motor to right sensor
    bl = geno[2];
    br = geno[3];
    sl_pos = np.zeros((2,1));
    sr_pos = np.zeros((2,1));
    ...
    for i in range(1, int(T/dt)):
        ...
        #w_ll = left sensor left motor
        #w_rl = left sensor right motor
        #lm = il*w_ll + bl;
        #rm = il*w_rl + br;
        lm = il*w_ll;
        rm = il*w_rl;
